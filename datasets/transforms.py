...
from misc.box_ops import box_xyxy_to_cxcy, box_xyxy_to_cxcywh
from misc.utils import interpolate
...
class Box2Point(object):
    ...
    def __call__(self, image, target):
        ...
        if "boxes" in target:
            boxes = target["boxes"]
            points = box_xyxy_to_cxcy(boxes)
            points = points / torch.tensor([w, h], dtype=torch.float32)
            target["points"] = points
            target["num"] = torch.tensor(len(points))
        return image, target


class ColorJitter(object):
    def __init__(self, p, brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2) -> None:
        super().__init__()
    ...
    def __call__(self, img, target):
        data = {}
        img = self.jitter(image=img)["image"]
...
class LongestMaxFilter(DualTransform):
    """Rescale an image so that maximum side is small to max_size, keeping the aspect ratio of the initial image.
    Args:
    ...
    def __init__(
        ...
        super(LongestMaxFilter, self).__init__(always_apply, p)
